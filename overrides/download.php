    ...
    <div class="<?php echo RSFilesAdapterGrid::card(); ?> mb-3">
        <div class="card-body">
			<?php if ($this->candownload) { ?>
                ...
                <p><?php if (in_array($extension, rsfilesHelper::previewExtensions()) && $this->file->show_preview) { ?>
                        <a href="javascript:void(0)"
                           onclick="rsfiles_show_modal('<?php echo JRoute::_('index.php?option=com_rsfiles&layout=preview&relatedfile=' . JFactory::getApplication()->input->get('relatedfile', 0) . '&tmpl=component&path=' . rsfilesHelper::encode($this->file->fullpath) . $this->itemid); ?>', '<?php echo JText::_('COM_RSFILES_PREVIEW'); ?>', <?php echo $size['height']; ?>, '<?php echo $properties['handler']; ?>');"
